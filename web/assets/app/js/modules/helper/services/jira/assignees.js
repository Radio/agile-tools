angular.module('helper')
    .factory('JiraHelper_Assignees', function(JiraHelper_TaskType) {
        return {
            getStoryAssignees: function(issue, version) {
                var assignees = {
                    devs: [],
                    tbd: {},
                    qa: {}
                ...
                if (issue.subtasks && issue.subtasks.length) {
                    ...
                    for (var i = 0; i < issue.subtasks.length; i++) {
                        ...
                        if (subTask.assignee && 'name' in subTask.assignee) {
                            setAssigneeByType(JiraHelper_TaskType.get(subTask), subTask.assignee);
                        }
                    }
                }
