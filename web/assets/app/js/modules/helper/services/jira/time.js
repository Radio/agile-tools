angular.module('helper')
    .factory('JiraHelper_Time', function(JiraHelper_TaskType, JiraHelper_Version) {
        ...
        return {
            getDetailedIssueTimeInfo: function(issue, version) {
                ...
                if (issue && issue.subtasks) {
                    for (var i = 0; i < issue.subtasks.length; i++) {
                        ...
                        if (!JiraHelper_Version.isInVersion(subTask, version)) {
                            continue;
                        }
                        if (!subTask.time) {
                            continue;
                        }
                        var subTaskType = JiraHelper_TaskType.get(subTask);
                        switch (subTaskType) {
                            case 'dev':
                            case 'inv':
                            case 'doc':
