angular.module('agile.controllers')
    .controller('Version_Resources', ['$rootScope', '$scope', '$location', 'Api', 'Helper', 'dateFormatFilter',
        function($rootScope, $scope, $location, Api, Helper, dateFormatFilter) {
            ...
            function synchronize()
            {
                ...
                loadVersionTasks().then(function(tasks) {
                    ...
                    for (var i = 0; i< tasks.length; i++) {
                        var task = tasks[i];
                        try {
                            var remaining = task.fields.timetracking.remainingEstimateSeconds;
                            var assignee = task.fields.assignee.name;
                    ...
                    $scope.plan.users.forEach(function(user) {
                        user.allocated = (stats[user.key] || 0) / 3600;
                    });
            ...
            function loadVersionTasks()
            {
                ...
                var request = {
                    _fields: "summary,timetracking,assignee",
